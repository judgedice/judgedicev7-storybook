...
export var QuoteCarousel = function (_a) {
    ...
    return (<section className="quote-carousel" style={styles}>
      <div className="quote-carousel__container">
        <div className={"quote-carousel__content ".concat(isTransitioning ? 'fade-out' : 'fade-in')}>
        ...
        <div className="quote-carousel__navigation">
          {quotes.map(function (_, index) { return (<button key={index} className={"quote-carousel__dot ".concat(index === currentIndex ? 'active' : '')} onClick={function () { return handleDotClick(index); }} aria-label={"View quote ".concat(index + 1)}/>); })}
        </div>
      </div>
    </section>);
};
